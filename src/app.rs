...
use crate::constants::{
    STAKING_POOL_ADDRESS, DEV_FUND_ADDRESS, LIQUIDITY_FUND_ADDRESS,
    BURN_PERCENTAGE, STAKING_PERCENTAGE, DEV_PERCENTAGE, 
    TRANSFER_FEE_DIVISOR, TRANSFER_FEE_MINIMUM
};
...
impl QuantumBlockchainApp {
    ...
    if !symbol.chars().all(|c| c.is_ascii_uppercase()) || symbol.len() < 2 || symbol.len() > 10 {
        return Err(anyhow::anyhow!("Símbolo do token deve ter entre 2 e 10 caracteres maiúsculos").into());
    }
    ...
    let kybl_balance = self.blockchain.tokens
        .get(&0.to_string())
    ...
    if kybl_balance < token_creation_fee {
        return Err(anyhow::anyhow!("Saldo insuficiente de KYBL para criar token. Mínimo: {}", token_creation_fee).into());
    }
    ...
    if token_count >= max_tokens_per_account {
        return Err(anyhow::anyhow!("Limite máximo de {} tokens por conta atingido", max_tokens_per_account).into());
    }
    
    // Cobra a taxa em KYBL
    let kybl_token = self.blockchain.tokens.get_mut(&0.to_string())
        .ok_or(anyhow::anyhow!("Token KYBL não encontrado"))?;
...
fn calculate_token_creation_fee(
    ...
    *kybl_token.balances.entry(STAKING_POOL_ADDRESS.to_string()).or_insert(0) += staking_amount;
    *kybl_token.balances.entry(DEV_FUND_ADDRESS.to_string()).or_insert(0) += dev_amount;
    *kybl_token.balances.entry(LIQUIDITY_FUND_ADDRESS.to_string()).or_insert(0) += liquidity_amount;
        ...
        let reputation = match self.database.get_connection() {
            Ok(conn) => {
                ...
                conn.query_row(
                    ...
                    [address],
                    |row| row.get::<_, i64>(0)
                ).unwrap_or(0) as u64 * 10 // 10 pontos por transação
            },
    ...
    pub fn transfer_token(&mut self, token: &mut CustomToken, to: String, amount: u64, secret_key: &dilithium5::SecretKey) -> Result<SecureTransaction> {
    // Validações básicas
    if amount == 0 {
    ...
    *kybl_token.balances.entry(from.clone()).or_insert(0) -= transfer_fee;
    ...
    info!("Transferência de {} tokens (ID: {}) para {}. Taxa: {} KYBL", 
