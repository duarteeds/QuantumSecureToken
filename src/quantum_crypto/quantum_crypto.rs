...
impl QuantumCrypto {
    ...
    pub fn verify_transaction(&self, transaction_data: &[u8], signature: &[u8]) -> Result<bool, OqsError> {
    self.validar_estado_interno()?;
    
    let public_key_bytes = STANDARD.decode(&self.data.public_key)
    ...
    let signature_bytes = STANDARD.decode(&self.data.signature)
...
fn validar_estado_interno(&self) -> Result<(), OqsError> {
    ...
    pub fn with_keys(
        ...
        if signature_scheme != "Dilithium5" {
            return Err(OqsError::ValidationError("Esquema de assinatura inválido".into()));
        }
    ...
    pub fn encrypt(&self, data: &[u8]) -> Result<(Vec<u8>, Vec<u8>, Secret<Vec<u8>>), OqsError> {
        ...
        Ok((encrypted_data, ciphertext.into_vec(), Secret::new(secret_key.into_vec())))
    }
    ...
    pub fn decrypt(
        ...
        let secret_key = self.kem.secret_key_from_bytes(secret_key.expose_secret())
            .ok_or(OqsError::CryptoError("Chave secreta inválida".into()))?;

        let ciphertext = self.kem.ciphertext_from_bytes(ciphertext)
    ...
    pub fn sign(&self, data: &[u8], secret_key: &Secret<Vec<u8>>) -> Result<Vec<u8>, OqsError> {
        let secret_key = self.sig.secret_key_from_bytes(secret_key.expose_secret())
            .ok_or(OqsError::CryptoError("Chave secreta inválida".into()))?;
    ...
    pub fn verify(&self, data: &[u8], signature: &[u8], public_key: &[u8]) -> Result<(), OqsError> {
        let public_key = self.sig.public_key_from_bytes(public_key)
            .ok_or(OqsError::CryptoError("Chave pública inválida".into()))?;
            
        let signature = self.sig.signature_from_bytes(signature)
            .ok_or(OqsError::CryptoError("Assinatura inválida".into()))?;

        self.sig.verify(data, &signature, &public_key)
    ...
    pub fn verify_str(&self, data: &[u8], signature: &str, public_key: &str) -> Result<(), OqsError> {
    ...
    pub fn rotacionar_chaves(&mut self, nova_chave: Vec<u8>) -> Result<(), OqsError> {
        if nova_chave.len() < 64 {
            return Err(OqsError::ValidationError("Tamanho de chave inválido".into()));
        }

        let meio = nova_chave.len() / 2;
        
        // Usar _ para indicar que a variável é intencionalmente não utilizada
        let _public_key = self.kem.public_key_from_bytes(&nova_chave[..meio])
            .ok_or(OqsError::CryptoError("Chave pública inválida".into()))?;
