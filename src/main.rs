...
// Adicionar este import para o trait PublicKey
...
fn main() -> Result<()> {
    ...
    // CORRIGIDO: Usando o método as_bytes() através do trait
    app.blockchain.public_keys.insert(
        admin_address.clone(), 
        public_key.as_bytes().to_vec()
    );
    
    // Se necessário, também registrar a chave secreta
    app.blockchain.secret_keys.insert(
        admin_address.clone(),
        secret_key.clone()
    );
    ...
    if let Some(kybl_token) = app.blockchain.tokens.get_mut(&0.to_string()) {
        ...
        *kybl_token.balances.entry(admin_address.clone()).or_insert(0) += 1_000_000;
        info!("Adicionado saldo inicial de 1,000,000 KYBL ao administrador: {}", admin_address);
    } else {
        error!("Token KYBL (ID 0) não encontrado ao tentar adicionar saldo inicial");
    }
    
    info!("Chaves do administrador registradas com sucesso: {}", admin_address);
    ...
    // Transferir tokens
    let transaction = app.transfer_token(
        &mut custom_token, 
        "0x456...".to_string(), 
        1000,
        &secret_key
