...
async fn process_epoch_transition(transition: &EpochTransition) {
    ...
    pub fn get_metrics(&self) -> ConsensusMetrics {
        ...
        let validators = self.validators.read().unwrap();
        let reputation = self.reputation.read().unwrap();
        let network = self.network_metrics.read().unwrap();

        ConsensusMetrics {
            consensus_type: state.consensus_type,
            current_block_height: state.current_block_height,
            active_validators: validators.count_active(),
            total_validators: validators.len(),
            average_reputation: reputation.average_reputation(),
            suspicious_validators: reputation.count_suspicious_validators(),
            network_condition: network.network_condition(),
            average_latency_ms: network.average_latency_ms,
            message_loss_rate: network.message_loss_rate,
            is_running: self.is_running(),
            current_epoch: state.current_epoch,
        }
    }

    /// Adiciona um novo validador ao conjunto
    pub fn add_validator(&self, validator: Validator) -> Result<bool, ConsensusError> {
        ...
        let result = validators_guard.add_validator(validator.clone());

        // Se adicionado com sucesso, adiciona ao sistema de reputação também
        if result {
            let mut rep_guard = self.reputation.write().map_err(|_| {
                ConsensusError::InternalError(
                    "Falha ao obter lock do sistema de reputação".to_string(),
                )
            })?;

            rep_guard.add_validator(validator.id.clone());

            info!(
                "➕ Validador {} adicionado com sucesso (stake: {})",
                validator.id, validator.stake
            );
        } else {
            debug!(
                "⚠️ Falha ao adicionar validador {} (stake insuficiente?)",
                validator.id
            );
        }
