...
impl KeyManager {
 ...
 pub fn delete_transaction_params(&self, from: &str, to: &str, amount: u64) -> Result<(), TransactionError> {
    if from.is_empty() || to.is_empty() {
        return Err(TransactionError::InvalidParameter("Endereço inválido".to_string()));
    }
    if amount == 0 {
        return Err(TransactionError::InvalidParameter("Quantidade inválida".to_string()));
    }
    Ok(())
}
