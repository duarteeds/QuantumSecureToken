...
pub fn size(&self) -> usize {
    self.code.len() + self.data.len() + self.address.len() + self.creator.len() + 8 // +8 para o timestamp
}
...
pub fn is_expired(&self, current_time: i64) -> bool {
    ...
    pub fn execute(&self, input: &str) -> Result<String, Box<dyn StdError>> {
        if self.quantum_secure {
            ...
            let module = Module::new(&store, &self.code).map_err(|e| {
                format!("Falha ao carregar o módulo Wasm: {}", e)
            })?;
            let import_object = imports! {};
            let instance = Instance::new(&module, &import_object).map_err(|e| {
                format!("Falha ao criar instância do módulo Wasm: {}", e)
            })?;
            let main_func = instance.exports.get_function("main").map_err(|e| {
                format!("Função 'main' não encontrada no contrato: {}", e)
            })?;
            let args = [Value::I32(input.len() as i32)];
            let result = main_func.call(&args).map_err(|e| {
                format!("Falha ao executar a função 'main': {}", e)
            })?;
            Ok(format!("{:?}", result))
        } else {
