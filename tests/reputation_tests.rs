...
fn test_reputation_ban_expires() {
    ...
    config.initial_ban_duration = std::time::Duration::from_millis(500); // Aumentado
    ...
    std::thread::sleep(std::time::Duration::from_millis(1000)); // Aumentado
    
    let reputation = system.get_reputation_mut("validator1").unwrap();
    let now = std::time::Instant::now();
    let banned_until = reputation.banned_until.as_ref().map(|u| u.to_instant());
    println!("Agora: {:?}, Banned até: {:?}", now, banned_until);
    let expired = reputation.check_ban_status();
    println!("Expired: {}, is_banned: {}", expired, reputation.is_banned);
