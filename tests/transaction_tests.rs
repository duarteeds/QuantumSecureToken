...
use kybelith::transaction::{Transaction, NonceRegistry};
use kybelith::constants::{MIN_ADDRESS_LENGTH, MAX_AMOUNT, MAX_TRANSACTION_SIZE};
use kybelith::error::TransactionError;
use pqcrypto_dilithium::dilithium5::{SecretKey, PublicKey, keypair, detached_sign};
...
fn serialize_transaction_for_signing(transaction: &Transaction) -> Result<Vec<u8>, TransactionError> {
    #[derive(Serialize)]
...
fn test_same_address() -> Result<(), TransactionError> {
    ...
    match result {
        Ok(transaction) => {
            ...
            assert!(validation_result.is_err(), "Transação com mesmo endereço de origem e destino deveria falhar na validação");
        },
        Err(err) => {
